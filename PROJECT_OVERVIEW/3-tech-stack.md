...
│   ├── identity/                     # Auth + Users + Organizations + Departments + Approval Chains (TCP :3010)
│   ├── recruiting/                   # HiringRequests + Roles + Applications + Invites + Evaluations (TCP :3011)
...
│   └── src/                          # Enterprise hiring workflow UI
│
...
│   └── ai/                           # Interview focus generation, schema validation
...
| **Routing** | React Router v7 | Flow-based SPA navigation (feeds → detail → evidence) |
| **UI Components** | Radix UI | Accessible primitives, CSS variable theming |
| **Styling** | Tailwind CSS + CSS Variables | Tailwind utilities for layout/spacing + "Case Review Ivory" CSS variable tokens for theming |
| **Data Tables** | TanStack Table | Headless, sortable, filterable candidate/job lists |
| **Command Palette** | CMDK | Keyboard-first navigation (Linear-style) |
...
| **Database** | PostgreSQL + JSONB | Relational integrity + flexible evidence metadata |
| **Vector Search** | pgvector | Semantic similarity without external service |
| **Queue** | Redis + BullMQ | Async parsing, embedding, evaluation jobs |
| **Auth** | JWT + refresh tokens | Stateless auth with role-based guards |
| **File Processing** | pdf-parse, mammoth | CV/JD extraction from PDF and DOCX |

## Design System: "Case Review Ivory" ✅ (Implemented)

Warm, low-glare light theme for long technical review sessions. All tokens defined in `packages/ui/src/styles/tokens.css`.
...
| **Readiness** | `--wr-readiness-*` | 7 colors mapped to ReadinessLabel enum values |
| **Typography** | `--wr-font-*` | IBM Plex Sans (UI) + IBM Plex Mono (evidence/provenance) |
...
| **StatusBadge** | custom | Readiness label-specific styling |
