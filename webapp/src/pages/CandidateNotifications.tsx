...
export const CandidateNotifications: React.FC = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<'All' | 'Unread' | 'Interviews' | 'Applications' | 'System'>('All');
  ...
  const handleToggleRead = (id: string, e: React.MouseEvent) => {
    ...
    setAlerts(prevAlerts =>
      prevAlerts.map(alert =>
        alert.id === id ? { ...alert, unread: !alert.unread } : alert
      )
    );
  };
  ...
  return (
    <div className="mx-auto max-w-[1440px] flex flex-col h-[calc(100vh-100px)]">
      ...
      <header className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight text-deep-charcoal mb-1">Inbox Alerts</h1>
          <p className="text-sm text-slate-ink">Check incoming notifications, interview invitations, and status updates.</p>
        </div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6" aria-label="Inbox Summary">
        ...
        <div 
          ...
          className={`bg-white border p-4 flex flex-col gap-1 relative overflow-hidden group cursor-pointer hover:border-teal-command transition-all rounded-lg ${
            activeFilter === 'Unread' ? 'border-teal-command ring-1 ring-teal-command/20 shadow-sm' : 'border-border-warm shadow-sm'
          }`}
        >
        ...
        <div 
          ...
          className={`bg-white border p-4 flex flex-col gap-1 relative overflow-hidden group cursor-pointer hover:border-approved transition-all rounded-lg ${
            activeFilter === 'Interviews' ? 'border-approved ring-1 ring-approved/20 shadow-sm' : 'border-border-warm shadow-sm'
          }`}
        >
        ...
        <div 
          ...
          className={`bg-white border p-4 flex flex-col gap-1 relative overflow-hidden group cursor-pointer hover:border-pending transition-all rounded-lg ${
            activeFilter === 'Applications' ? 'border-pending ring-1 ring-pending/20 shadow-sm' : 'border-border-warm shadow-sm'
          }`}
        >
        ...
        <div 
          ...
          className={`bg-white border p-4 flex flex-col gap-1 relative overflow-hidden group cursor-pointer hover:border-tertiary transition-all rounded-lg ${
            activeFilter === 'System' ? 'border-tertiary ring-1 ring-tertiary/20 shadow-sm' : 'border-border-warm shadow-sm'
          }`}
        >
        ...
        <div className="w-full md:w-[40%] flex flex-col border-r border-border-warm bg-workflow-ivory/30 h-full overflow-hidden">
          ...
          <div className="p-3 border-b border-border-warm bg-white shrink-0 overflow-x-auto no-scrollbar">
            <div className="flex items-center gap-2 min-w-max">
              {(['All', 'Unread', 'Interviews', 'Applications', 'System'] as const).map((filter) => (
                <button
                  key={filter}
                  ...
                  >
                    ...
                    <div className="pl-3 flex flex-col gap-1">
                      <div className="flex justify-between items-start mb-1 gap-2">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full font-semibold text-[10px] uppercase tracking-wide ${
                        ...
                        <span className="text-[11px] font-semibold text-slate-ink shrink-0">{alert.receivedText}</span>
                      </div>
                      
                      <h3 className={`text-sm text-deep-charcoal truncate ${alert.unread ? 'font-bold' : 'font-semibold'}`}>
                        {alert.title}
                      </h3>
            ...
            <div className="flex flex-col h-full overflow-hidden">
              ...
              <div className="p-6 border-b border-border-warm flex justify-between items-start shrink-0 bg-workflow-ivory/10">
                <div className="flex flex-col gap-2">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full font-semibold text-xs border w-fit ${
                  ...
                  <h2 className="text-xl font-semibold text-deep-charcoal mt-1">{selectedAlert.title}</h2>
                  <p className="text-xs text-slate-ink font-medium">Received: {selectedAlert.receivedText}</p>
                </div>
              ...
              <div className="p-6 flex-1 overflow-y-auto flex flex-col gap-6">
                ...
                {selectedAlert.relatedJob && (
                  <div className="bg-workflow-ivory border border-border-warm rounded-lg p-4 flex items-center justify-between group cursor-pointer hover:border-teal-command transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-slate-ink uppercase tracking-wider">Related Job Application</p>
                        <h4 className="text-sm font-semibold text-deep-charcoal mt-0.5">{selectedAlert.relatedJob}</h4>
                      </div>
                    </div>
                ...
                {selectedAlert.type === 'Interview' && selectedAlert.date && (
                  <div className="bg-surface-container-low border border-border-warm rounded-lg p-4 flex flex-col gap-3 mt-2 shadow-sm">
                    <h5 className="text-xs font-bold uppercase tracking-wider text-deep-charcoal mb-1">Interview Details</h5>
