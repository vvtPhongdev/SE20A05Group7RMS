...
const weekSlots: WeekSlot[] = [
  {
    ...
    date: 'OCT 23',
    items: [
      { time: '09:00 - 10:00', candidate: 'Sarah Jenkins', role: 'Product Designer', tone: 'Scheduled' },
  ...
  },
  {
    day: 'TUE',
    date: 'OCT 24',
    items: [{ time: '11:00 - 12:00', candidate: 'Lydia Chuo', role: 'Cancelled', tone: 'Cancelled' }],
  },
  {
    day: 'WED',
    date: 'OCT 25',
    items: [
      { time: '10:00 - 11:30', candidate: 'Jonathan Reeve', role: 'Marketing Lead', tone: 'Scheduled' },
      { time: '16:00 - 17:00', candidate: 'Elena Fisher', role: 'Rescheduled', tone: 'Rescheduled' },
    ],
  },
];

const panel = [
  { name: 'David Miller', status: 'Conflict: 10:00 AM', tone: 'text-error', surface: 'bg-error-container/20' },
  { name: 'Aisha Khan', status: 'Available', tone: 'text-approved', surface: 'bg-surface-container-low' },
  { name: 'Kevin Smith', status: 'Available after 14:00', tone: 'text-revision', surface: 'bg-revision/10' },
];
    ...
    <div className="mx-auto max-w-[1440px] space-y-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold tracking-tight text-deep-charcoal">Interview Schedule</h1>
      ...
      <section className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4" aria-label="Interview schedule metrics">
        {kpis.map((kpi) => (
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[280px_minmax(0,1fr)_340px]">
        <section className="overflow-hidden rounded-lg border border-border-warm bg-clean-surface shadow-sm xl:sticky xl:top-24">
          <div className="flex items-center justify-between border-b border-border-warm bg-parchment-lift/40 p-4">
            ...
            <div className="flex gap-2">
              <button className="rounded-lg p-1.5 transition hover:bg-surface-container active:scale-[0.98]" type="button" aria-label="Previous week">
                <Icon className="h-4 w-4" name="chevronLeft" />
              </button>
              <button className="rounded-lg p-1.5 transition hover:bg-surface-container active:scale-[0.98]" type="button" aria-label="Next week">
            ...
            {weekSlots.map((day, dayIndex) => (
              <div key={`${day.day}-${day.date}`}>
                ...
                <div className="space-y-2">
                  {day.items.map((item) => (
                    <article className={`rounded-lg border p-3 ${slotClass[item.tone]}`} key={`${day.day}-${item.time}-${item.candidate}`}>
                      <p className="font-mono text-xs font-semibold">{item.time}</p>
                      <p className="mt-1 text-sm font-semibold text-deep-charcoal">{item.candidate}</p>
        ...
        <main className="min-w-0 space-y-4">
          <section className="overflow-hidden rounded-lg border border-border-warm bg-clean-surface shadow-sm">
            <div className="flex flex-col gap-3 border-b border-border-warm p-4 lg:flex-row lg:items-center lg:justify-between">
              <h2 className="text-sm font-semibold uppercase tracking-[0.14em] text-deep-charcoal">Active Interview Queue</h2>
              <div className="flex flex-wrap gap-2">
                {(['All', 'Scheduled', 'Rescheduled', 'Cancelled', 'Completed'] as const).map((status) => (
                  <button
                ...
                <button className="inline-flex items-center gap-2 rounded-lg border border-border-warm px-3 py-1.5 text-xs font-semibold transition hover:bg-surface-container" type="button">
              ...
              <table className="w-full min-w-[760px] border-collapse">
                ...
                <tbody className="divide-y divide-border-warm/60">
                  {visibleInterviews.map((interview, index) => (
                    <tr className={`${index % 2 === 1 ? 'bg-workflow-ivory/30' : ''} transition hover:bg-surface-container-low`} key={interview.id}>
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-deep-charcoal">{interview.candidate}</p>
                      ...
                      <td className="p-4">
                        <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-[11px] font-bold uppercase tracking-tight ${statusClass[interview.status]}`}>
          ...
          <section className="flex items-start gap-3 rounded-lg border border-border-warm bg-parchment-lift/50 p-4">
            ...
            <p className="text-sm italic leading-6 text-slate-ink">
              Scheduling requires an approved request and an approved overall campaign plan. Verify all panel member calendars before dispatching invites.
            </p>
          </section>
        </main>
        ...
        <aside className="overflow-hidden rounded-lg border border-border-warm bg-clean-surface shadow-sm xl:sticky xl:top-24">
          <div className="bg-teal-command p-5 text-white">
            <h2 className="text-sm font-bold uppercase tracking-[0.16em]">Schedule New Interview</h2>
          </div>
          <form className="space-y-4 p-5" onSubmit={(event) => event.preventDefault()}>
            <label className="block space-y-1.5">
              <span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-ink">Candidate</span>
            ...
            <label className="block space-y-1.5">
              <span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-ink">Interviewers</span>
              <select
              ...
              <span className="text-xs text-slate-ink">Hold Ctrl/Cmd to select multiple members.</span>
            </label>
            <div className="grid grid-cols-2 gap-3">
              <label className="block space-y-1.5">
                <span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-ink">Date</span>
                <input className="w-full rounded-lg border border-border-warm bg-workflow-ivory p-2 text-sm outline-none focus:border-teal-command focus:ring-2 focus:ring-teal-command/20" type="date" />
              </label>
              <label className="block space-y-1.5">
                <span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-ink">Time</span>
                <input className="w-full rounded-lg border border-border-warm bg-workflow-ivory p-2 text-sm outline-none focus:border-teal-command focus:ring-2 focus:ring-teal-command/20" type="time" />
              </label>
            </div>
            <label className="block space-y-1.5">
              <span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-ink">Duration</span>
              <select className="w-full rounded-lg border border-border-warm bg-workflow-ivory p-2.5 text-sm outline-none focus:border-teal-command focus:ring-2 focus:ring-teal-command/20" defaultValue="60 Minutes">
            ...
            <label className="block space-y-1.5">
              <span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-ink">Meeting URL / Location</span>
              <input
            ...
            <section className="border-t border-border-warm pt-4">
              <div className="mb-2 flex items-center justify-between">
                <h3 className="text-xs font-bold uppercase tracking-[0.14em] text-slate-ink">Panel Availability</h3>
                <Icon className="h-4 w-4 text-revision" name="warning" />
              </div>
              <div className="space-y-2">
                {panel.map((member) => (
                  <div className={`flex items-center justify-between gap-3 rounded-lg p-2 text-xs ${member.surface}`} key={member.name}>
            ...
            <div className="flex flex-col gap-3 pt-2">
              ...
              <button className="w-full rounded-lg bg-teal-command py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary active:scale-[0.98]" type="submit">
                Send Invitation
              </button>
            </div>
          </form>
