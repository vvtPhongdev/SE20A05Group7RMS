...
export const PlaceholderPage: React.FC<PlaceholderPageProps> = ({ title, description }) => {
  ...
  const inferredDescription =
    description ||
    `This screen represents the interface for ${inferredTitle.toLowerCase()}. The underlying recruitment microservices and APIs are fully mapped and ready for data integration.`;

  return (
    <div className="flex flex-col max-w-[800px] animate-[fadeIn_0.3s_ease-out]">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[var(--wr-text-primary)] mt-0 mb-2">
          {inferredTitle}
        </h1>
        <p className="text-base text-[var(--wr-text-secondary)] m-0 leading-relaxed">
          {inferredDescription}
        </p>
      </div>
      ...
      <div className="bg-[var(--wr-bg-surface)] border border-[var(--wr-border-default)] rounded-[var(--wr-radius-lg)] p-8 shadow-[var(--wr-shadow-md)] flex flex-col relative overflow-hidden">
        ...
        <h2 className="text-lg font-semibold text-[var(--wr-text-primary)] mt-0 mb-3">
          System Workspace Connected
        </h2>
        <p className="text-sm text-[var(--wr-text-secondary)] mt-0 mb-8 leading-normal">
          All gateway routes and RBAC rules for path <code>{location.pathname}</code> are
          configured. The backend services are listening for incoming TCP requests.
        </p>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-6">
          <div className="flex flex-col gap-1">
            <span className="text-xs text-[var(--wr-text-muted)] font-medium uppercase">
              Route Target
            </span>
            <span className="text-sm text-[var(--wr-text-primary)] font-semibold">
              {location.pathname}
            </span>
          </div>
          <div className="flex flex-col gap-1">
