...
const kpis = [
  { label: 'Total Candidates', value: '2,480', helper: '+12% vs last month', tone: 'text-approved' },
  ...
  { label: 'In Active Campaigns', value: '856', helper: 'Across 14 open roles', tone: 'text-deep-charcoal' },
];
...
export const TalentPool: React.FC = () => {
  ...
  const visibleCandidates = useMemo(() => {
    ...
    return candidates.filter((candidate) => {
      const matchesQuery =
        !normalizedQuery ||
        [candidate.name, candidate.title, candidate.role, candidate.stage, candidate.email, ...candidate.skills].some((value) =>
          value.toLowerCase().includes(normalizedQuery),
        );
      const matchesRole = role === 'All Roles' || candidate.role === role;
      const matchesAvailability = availability === 'All Availability' || candidate.availability === availability;
      ...
      return matchesQuery && matchesRole && matchesAvailability && matchesParse && matchesEligibility;
    });
  }, [availability, eligibleOnly, parseStatus, query, role]);

  const selected = candidates.find((candidate) => candidate.id === selectedId) ?? visibleCandidates[0] ?? candidates[0];
    ...
    <div className="mx-auto grid max-w-[1440px] gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      <main className="min-w-0 space-y-6">
        <header className="grid gap-4 xl:grid-cols-[1fr_minmax(260px,420px)] xl:items-end">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-command">HR Manager Workspace</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight text-deep-charcoal">Talent Pool</h1>
            <p className="mt-1 max-w-[70ch] text-sm leading-6 text-slate-ink">
              Browse parsed CVs, inspect structured candidate data, and prepare eligible profiles for active campaigns.
            </p>
          </div>
        ...
        <section className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4" aria-label="Talent pool metrics">
          {kpis.map((kpi) => (
            <section className="rounded-lg border border-border-warm bg-clean-surface p-5 shadow-sm" key={kpi.label}>
              ...
              <div className="mt-3 flex items-baseline gap-2">
                <span className={`font-mono text-[32px] font-semibold leading-none ${kpi.tone}`}>{kpi.value}</span>
              </div>
        ...
        <section className="rounded-lg border border-border-warm bg-clean-surface p-4 shadow-sm">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_1fr_auto] xl:items-end">
            <label className="flex flex-col gap-1">
              ...
              <select className="h-10 rounded-lg border border-border-warm bg-clean-surface px-3 text-sm outline-none focus:border-teal-command focus:ring-2 focus:ring-teal-command/20" onChange={(event) => setRole(event.target.value)} value={role}>
            ...
            <label className="flex flex-col gap-1">
              ...
              <select className="h-10 rounded-lg border border-border-warm bg-clean-surface px-3 text-sm outline-none focus:border-teal-command focus:ring-2 focus:ring-teal-command/20" onChange={(event) => setAvailability(event.target.value)} value={availability}>
            ...
            <label className="flex flex-col gap-1">
              ...
              <select className="h-10 rounded-lg border border-border-warm bg-clean-surface px-3 text-sm outline-none focus:border-teal-command focus:ring-2 focus:ring-teal-command/20" onChange={(event) => setParseStatus(event.target.value)} value={parseStatus}>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                type="button"
              >
                <span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${eligibleOnly ? 'right-1' : 'left-1'}`} />
              </button>
              <span className="text-sm font-semibold text-on-surface-variant">Eligible</span>
              <button className="rounded-lg p-2 text-teal-command transition hover:bg-surface-container active:scale-[0.98]" onClick={resetFilters} type="button">
            ...
            return (
              <article
                className={`flex cursor-pointer flex-col rounded-lg bg-clean-surface p-5 transition hover:-translate-y-[2px] hover:shadow-md ${
                  active ? 'border-2 border-teal-command/30 ring-2 ring-teal-command/5' : 'border border-border-warm'
                }`}
                key={candidate.id}
              ...
              >
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div className="flex min-w-0 gap-3">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-surface-container text-xl font-bold text-teal-command">
                      {candidate.photo ? <img alt={candidate.name} className="h-full w-full object-cover" src={candidate.photo} /> : candidate.initials}
                    </div>
                    <div className="min-w-0">
                      <h2 className="truncate text-xl font-bold text-deep-charcoal">{candidate.name}</h2>
                      <p className="text-sm text-on-surface-variant">{candidate.title}</p>
                    </div>
                  </div>
                  <span className={`rounded px-2 py-1 text-[10px] font-bold uppercase tracking-[0.08em] ${statusClass[candidate.parseStatus]}`}>
                ...
                <div className="mb-4 flex flex-wrap gap-2">
                  {candidate.skills.slice(0, 4).map((skill) => (
                    <span className="rounded-full border border-border-warm bg-workflow-ivory px-2 py-1 text-xs text-slate-ink" key={skill}>
                      {skill}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-auto grid grid-cols-2 gap-3">
                  <button className={`h-10 rounded-lg border border-teal-command text-sm font-semibold text-teal-command transition hover:bg-teal-command/5 ${active ? 'bg-teal-command/5' : ''}`} type="button">
                    View Profile
                  </button>
        ...
        {visibleCandidates.length === 0 ? (
          <section className="rounded-lg border border-border-warm bg-clean-surface px-6 py-12 text-center">
            <p className="text-sm font-semibold text-deep-charcoal">No candidates match the current filters.</p>
            <p className="mt-1 text-sm text-slate-ink">Clear filters or broaden the search query.</p>
          </section>
        ) : null}
      </main>
      ...
      <aside className="min-w-0 rounded-lg border border-border-warm bg-parchment-lift shadow-lg xl:sticky xl:top-6 xl:self-start">
        <div className="flex items-center justify-between p-5">
          ...
          <button className="rounded-full p-1.5 text-on-surface-variant transition hover:bg-surface-container hover:text-deep-charcoal" onClick={() => setSelectedId('')} type="button">
        ...
        {selected ? (
          <>
            <div className="px-6 pb-6">
              <div className="mb-8 flex flex-col items-center text-center">
                <div className="mb-4 flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border-4 border-white bg-clean-surface text-2xl font-bold text-teal-command shadow-md">
                  {selected.photo ? <img alt={`${selected.name} profile`} className="h-full w-full object-cover" src={selected.photo} /> : selected.initials}
                </div>
              ...
              <div className="space-y-6">
                ...
                <section>
                  <h4 className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-on-surface-variant">Structured Skills</h4>
                  <div className="flex flex-wrap gap-2">
                    {selected.skills.map((skill) => (
                      <span className="rounded-lg border border-border-warm bg-white px-3 py-1.5 text-sm font-semibold text-deep-charcoal" key={skill}>
                        {skill}
                      </span>
                    ))}
                  </div>
                ...
                <section>
                  <h4 className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-on-surface-variant">Recent Applications</h4>
                  <div className="overflow-hidden rounded-lg border border-border-warm bg-white shadow-sm">
                    <div className="flex items-center justify-between gap-3 p-3">
                      ...
                      <span className="rounded bg-pending/5 px-2 py-1 text-[11px] font-bold text-pending">Ongoing</span>
                    </div>
                  </div>
                </section>
            ...
            <footer className="space-y-4 border-t border-border-warm bg-workflow-ivory p-6">
              ...
              <button className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-teal-command font-bold text-white transition hover:bg-primary active:scale-[0.98]" type="button">
          ...
          <div className="px-6 pb-8 text-center">
            ...
            <p className="mt-1 text-sm text-slate-ink">Choose a card to inspect parsed profile data.</p>
          </div>
        )}
      </aside>
    </div>
