...
const initialFeedback: PanelFeedback[] = [
  {
    ...
    culture: 9,
    notes:
      'Strong backend depth and system design clarity. Needs a short follow-up on ownership style in incident response.',
  },
  {
    id: 'sophia',
    member: 'Sophia Tan',
    ...
    culture: 5,
    notes:
      'Good technical vocabulary, but examples felt less aligned with cross-functional product tradeoffs.',
  },
];
      ...
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-[72ch] text-sm leading-6 text-slate-ink">
            Record panel feedback, final recommendation, and supporting evidence after completed
            interviews.
          </p>
        </div>
        <div className="relative w-full max-w-md">
          <Icon
            className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-ink"
            name="search"
          />
          <input
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
        ...
        <main className="overflow-hidden rounded-lg border border-border-warm bg-clean-surface shadow-sm">
          ...
          <footer className="flex flex-col gap-3 border-t border-border-warm bg-clean-surface p-5 sm:flex-row sm:items-center sm:justify-between">
            <button
              className="text-sm font-semibold text-slate-ink transition hover:text-deep-charcoal"
              type="button"
            >
              Cancel
            </button>
            <div className="flex flex-col gap-3 sm:flex-row">
              <button
                className="rounded-lg border border-teal-command px-6 py-2.5 text-sm font-semibold text-teal-command transition hover:bg-teal-command/5 active:scale-[0.98]"
                type="button"
              >
                Save as Draft
              </button>
              <button
                className="rounded-lg bg-teal-command px-8 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary active:scale-[0.98]"
                type="button"
              >
