...
export const HRDashBoard: React.FC = () => {
  ...
  const visiblePlans = useMemo(() => {
    ...
    return activePlans.filter((plan) => {
      ...
      const matchesQuery =
        !normalizedQuery ||
        [plan.id, plan.position, plan.department, plan.phase, plan.owner].some((value) => value.toLowerCase().includes(normalizedQuery));
      ...
      <header className="grid gap-4 xl:grid-cols-[1fr_auto] xl:items-end">
        <div>
          ...
          <p className="mt-1 max-w-[68ch] text-sm leading-6 text-slate-ink">
            Monitor approved requests, active recruitment plans, interview load, and candidate flow from one work surface.
          </p>
        </div>
          ...
          <DashboardCard className="overflow-hidden p-0">
              ...
              <table className="w-full min-w-[900px] text-left">
                ...
                <tbody className="divide-y divide-border-warm text-sm">
                  {visiblePlans.map((plan, index) => (
                    <tr className={`transition hover:bg-workflow-ivory/70 ${index % 2 ? 'bg-workflow-ivory/30' : 'bg-clean-surface'}`} key={plan.id}>
                      ...
                      <td className="px-5 py-4">
                        <div className="w-36">
                          <div className="h-1.5 overflow-hidden rounded-full bg-surface-container">
                            <div className={`h-full rounded-full ${progressStyles[plan.phase]}`} style={{ width: `${plan.progress}%` }} />
                          </div>
            ...
            {visiblePlans.length === 0 ? (
              <div className="border-t border-border-warm px-6 py-12 text-center">
                <p className="text-sm font-semibold text-deep-charcoal">No recruitment plans match this view.</p>
                <p className="mt-1 text-sm text-slate-ink">Adjust the search term or phase filter.</p>
              </div>
            ) : null}
          </DashboardCard>
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                <span className="inline-flex items-center gap-2 text-sm font-semibold">
                  ...
                  2 actions require immediate attention
                </span>
            ...
            <div className="flex h-48 items-end justify-center gap-4 rounded-lg bg-workflow-ivory p-4">
              {pipelineBars.map((bar) => (
                <div className="flex h-full flex-1 max-w-10 flex-col items-center justify-end gap-2" key={bar.label}>
                  <div className="relative w-full rounded-t-sm bg-teal-command/20" style={{ height: bar.height }} title={`${bar.label}: ${bar.value}`}>
                    <div className="absolute inset-x-0 bottom-0 rounded-t-sm bg-teal-command transition-all duration-300 hover:h-full" style={{ height: bar.fill }} />
                  </div>
