...
def create_weekly_docx(week_name, tasks):
    ...
    for idx, task in enumerate(tasks):
        ...
        heading_run = heading.add_run(f"Task {idx + 1}: {task['Task / Activity']}")
        ...
        metadata = [
            ("AI Tool Used", task["AI Tool Used"]),
            ("SDLC Phase", task["SDLC Phase"]),
            ("Quantitative Measure", task["Quantitative Measure"]),
            ("Value Added (1-5)", str(task["Value Added (1-5)"])),
            ("Risks & Limitations", task["Risks / Limitations Observed"])
        ]
        ...
        p_head_run = p_head.add_run("1. Prompt Sent to AI (Prompt)")
        ...
        r_head_run = r_head.add_run("2. Detailed AI Response")
        ...
        v_head_run = v_head.add_run("3. Student's Validation, Selection & Source Code Customization")
...
if __name__ == "__main__":
    print("Converting weekly reports to Word documents in English...")
    for week_name, tasks in WEEKLY_DATA.items():
        create_weekly_docx(week_name, tasks)
    print("All Word documents generated successfully in English.")
